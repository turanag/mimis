...
int DoConnect()
{
	...
	//device=*(BLUETOOTH_DEVICE_INFO*)lpParam;

	BLUETOOTH_DEVICE_INFO device=Get_BLUETOOTH_DEVICE_INFO();
	ifstream ix("bdi.out",ios::binary);
	ix.read(reinterpret_cast<char*>(&device),sizeof(device));
	...
	bthSockAddr.btAddr=device.Address.ullLong;
	bthSockAddr.port=0x11; //BT_PORT_ANY;
	...
	else
	{
		info="Connected to: ";
		//info+=device.szName;
		MessageBox(NULL,info.c_str(),"Connect",MB_OK);
	}

	return 0;
}

BOOL RegFun(LPVOID p,PBLUETOOTH_DEVICE_INFO pDevice)
{
	cout << "\n## RegFun called backed...";
	return 0;
}
...
//return 0;
	...
	USBupdown(3,true,dbg);
	//Sleep(1000);
	//return DoConnect();½

//	USBupdown(3,false,dbg);

	BLUETOOTH_DEVICE_INFO bdi=Get_BLUETOOTH_DEVICE_INFO();
	ifstream ix("bdi.out",ios::binary);
	ix.read(reinterpret_cast<char*>(&bdi),sizeof(bdi));
...
BluetoothEnableIncomingConnections(hRadio(),1);
/*
for(int ii=0;ii<6;++ii) bdi.Address.rgBytes[ii]=0;

	HBLUETOOTH_AUTHENTICATION_REGISTRATION reg;
    status=BluetoothRegisterForAuthentication(&bdi,&reg,RegFun,0);    
*/
	...
	// Lenovo BT adr= 00:50:B6:A0:48:8C
	...
	for(int i=0;i<6;++i) cout << dec << " [" << i << "]=" << hex << pass[i];

	status=BluetoothAuthenticateDevice(NULL,hRadio(),&bdi,pass,6);
	//status=BluetoothAuthenticateDevice(NULL,hRadio(),&bdi,0,0);

	// status=BluetoothAuthenticateDeviceEx(); // only found on Vista

	if (status==ERROR_SUCCESS) cout << "** OK: BluetoothAuthenticateDevice";
	else                       cout << "** ERROR: BluetoothAuthenticateDevice, " << status;

	if (status==ERROR_INVALID_PARAMETER) cout << "(ERROR_INVALID_PARAMETER)";
	if (status==ERROR_NO_MORE_ITEMS)        cout << "(BTH_ERROR_NO_CONNECTION)";


	if (status==BTH_ERROR_AUTHENTICATION_FAILURE) cout << "(BTH_ERROR_AUTHENTICATION_FAILURE)";
	if (status==BTH_ERROR_NO_CONNECTION)          cout << "(BTH_ERROR_NO_CONNECTION)";
	if (status==BTH_ERROR_PAGE_TIMEOUT	)         cout << "(BTH_ERROR_PAGE_TIMEOUT	)";
	if (status==BTH_ERROR_HARDWARE_FAILURE	)     cout << "(BTH_ERROR_HARDWARE_FAILURE	)";
	if (status==BTH_ERROR_AUTHENTICATION_FAILURE) cout << "(BTH_ERROR_AUTHENTICATION_FAILURE)";
	if (status==BTH_ERROR_MEMORY_FULL	)         cout << "(BTH_ERROR_MEMORY_FULL	)";
	if (status==BTH_ERROR_CONNECTION_TIMEOUT	) cout << "(BTH_ERROR_CONNECTION_TIMEOUT	)";
	if (status==BTH_ERROR_LMP_RESPONSE_TIMEOUT)   cout << "(BTH_ERROR_LMP_RESPONSE_TIMEOUT)";
	if (status==BTH_ERROR_MAX_NUMBER_OF_CONNECTIONS	)   cout << "(BTH_ERROR_MAX_NUMBER_OF_CONNECTIONS)";
	if (status==BTH_ERROR_PAIRING_NOT_ALLOWED	)   cout << "(BTH_ERROR_PAIRING_NOT_ALLOWED	)";
	if (status==BTH_ERROR_UNSPECIFIED_ERROR	)   cout << "(BTH_ERROR_UNSPECIFIED_ERROR)";
	if (status==BTH_ERROR_LOCAL_HOST_TERMINATED_CONNECTION	)   cout << "(BTH_ERROR_LOCAL_HOST_TERMINATED_CONNECTION)½";

	if (status==ERROR_SUCCESS) {
		cout << "Running wb...\n";
		int n= system("D:\\wiiscan-0.9-release-2.0\\bin\\Release\\WiimoteWhiteboard.exe");
		cout << "  returned n=" << n << "\n";
	}
...
/*
	...
	status=BluetoothSetServiceState(hRadio(),&bdi,&service,BLUETOOTH_SERVICE_ENABLE);
	if      (dbg && status==ERROR_SUCCESS) log << "  BluetoothSetServiceState(0)...OK" << endl;
	else if (dbg && status==E_INVALIDARG)  log << "  BluetoothSetServiceState(0)...E_INVALIDARG" << endl;
	else if (dbg && status!=ERROR_SUCCESS) log << "  BluetoothSetServiceState(0)...ERROR" << endl;
...
/*
	...
	//return Wiilib::OpenDevices("wiiuse",4000,false,true,false);

	WCHAR pass[]=L"1234";
	return BluetoothAuthenticateDevice(NULL,hRadio(),&bdi,pass,4);

	if (dbg) log << tostring(bdi);

	status=BluetoothSetServiceState(hRadio(),&bdi,&service,BLUETOOTH_SERVICE_ENABLE);
	if      (dbg && status==ERROR_SUCCESS) log << "  BluetoothSetServiceState(1)...OK" << endl;
	else if (dbg && status==E_INVALIDARG)  log << "  BluetoothSetServiceState(1)...E_INVALIDARG" << endl;
	else if (dbg && status!=ERROR_SUCCESS) log << "  BluetoothSetServiceState(1)...ERROR" << endl;
*/
...
return TestOpen(NINTENDO_DEV,btr,true);
...
const int MAX_RADIOS=1;
const int MAX_DEVICES=16;
const int MAX_MAC_ADDR_LENGHT=256;
const int MAX_STRING_LENGHT=1024;

string GetLastErrorMessage(const int n) {return "error n";}

// PFN_DEVICE_CALLBACK

BOOL WINAPI RegFun2(LPVOID pvParam,const BLUETOOTH_DEVICE_INFO* pDevice)
{
	cout << "\n**RegFun2**\n";
	return 1;
}
